...
public class LteHdImsiProvinceConfigDao {

	private static Logger logger = LoggerFactory.getLogger(LteHdImsiProvinceConfigDao.class); // 日志
	
	/**
	 * 读取lte话单汇总IMSI与省份关联配置数据(数据库地址)语句
	 */
	private String sqlForLteHdSummaryImsiProvinceConfigDB;

	public String getSqlForLteHdSummaryImsiProvinceConfigDB() {
		return sqlForLteHdSummaryImsiProvinceConfigDB;
	}

	public void setSqlForLteHdSummaryImsiProvinceConfigDB(
			String sqlForLteHdSummaryImsiProvinceConfigDB) {
		this.sqlForLteHdSummaryImsiProvinceConfigDB = sqlForLteHdSummaryImsiProvinceConfigDB;
	}
	...
	public SummarySystemConfig getCommonConfg() {
		logger.debug("开始执行imsi与省份配置数据库配置加载!");
		...
		try {
			...
			statement = conn.prepareStatement(sqlForLteHdSummaryImsiProvinceConfigDB);
			rs = statement.executeQuery();
			while (rs.next()) {
				summarySystemConfig = new SummarySystemConfig();
				// 读取外部FTP配置
				summarySystemConfig.setConnectionInfo(getExtraDB(rs));
				logger.debug("imsi与省份配置数据库配置igp_cfg_connection,IGP_CFG_CONNECTION_DB查询成功!");
			}
			if (summarySystemConfig == null) {
				ImportantLogger.getLogger().warn("imsi与省份配置数据库配置igp_cfg_connection,IGP_CFG_CONNECTION_DB未配置!");
